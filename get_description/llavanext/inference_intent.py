...
def parse_args():
    ...
    # 定义命令行参数
    parser.add_argument("--video_path", help="Path to the video files.", default=".mp4")
    parser.add_argument("--video_folder", help="Path to the folder containing video files.", default='')  # 新增参数
    parser.add_argument("--input_csv", help="Path to the input CSV file.", default='')  # 新增参数
    parser.add_argument("--output_results", help="Path to the output results JSON file.", default='')  # 新增参数
...
def run_inference(args):
    ...
    output_results = args.output_results  # 新增：输出结果文件路径
    results_dict = {}  # 用于存储视频名和推理结果的字典
    ...
    for video_name in tqdm(video_names_in_csv, desc="Processing videos"):
        if video_name in video_folder_files_set:
            ...
            # 将结果添加到字典中
            results_dict[video_name] = outputs
